...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) payable ERC721("Octy Name Service", "ONS") {
    tld = _tld;
  ...
  function register(string calldata name) public payable {
    ...
    string memory finalTokenUri = string( abi.encodePacked("data:image/svg+xml;base64,", json));
  ...
  function valid(string calldata name) public pure returns(bool) {
    return StringUtils.strlen(name) >= 1 && StringUtils.strlen(name) <= 16;
  } 
}